...
void dumplolo(FILE * srcfile, FILE * outfile);
void dumppoem(FILE * srcfile, FILE * outfile, FILE * romfile, int model);
...
int ihexconv(FILE * srcfile, FILE * outfile, int mode, FILE * romfile, int model)
{
  ...
  switch (mode) {
    ...
    break;
    case MODE_LOLO:
      dumplolo(srcfile, outfile);
    break;
    case MODE_POEM:
      dumppoem(srcfile, outfile, romfile, model);
    default:
    break;
  }
...
void dumplolo(FILE * srcfile, FILE * outfile)
{
  ...
  if (cols != 0) {
    while (cols++ < 8) fprintf(outfile, "00");
    fprintf(outfile, ":%02X\n", (int)chksum);
  }
}
...
void dumppoem(FILE * srcfile, FILE * outfile, FILE * romfile, int model)
{
  ...
  for (i=0; i<180; i++) {
    romch = fgetc(romfile);
    if (romch == EOF) {
      fprintf(stderr, "End of ROM file reached in poems header");
      return;
    }
    fputc(romch, outfile);
  }
  
  if (model == MODEL_1350) i = 0;
    else i = 0x2000;
  ...
  if (ch != EOF) fprintf(stderr, "End of ROM file reached before end of memory image");
}
